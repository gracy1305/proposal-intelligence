...
def extract_proposal_fields(text: str) -> dict:
    ...
    Uses deterministic regex patterns so extraction remains explainable.
    """
    ...
    patterns = {
        ...
        "resort_fee": [
            ...
            r"\s*(?:is|of|at|:|-)?\s*"
            r"(\$[\d,]+(?:\.\d{1,2})?"
            r"(?:\s*per\s+(?:room|night)(?:\s*per\s*night)?)?)",
        ],
        ...
        "expiration_date": [
            r"(?:proposal\s*)?"
            r"(?:expires?|expiration(?:\s*date)?|valid\s*until)"
            r"\s*(?:is|on|:|-)?\s*"
    ...
    cancellation_patterns = [
        r"(Cancellation\s+(?:is|terms?\s*(?:are|:)?|policy\s*(?:is|:)?)"
        r"\s*[^\n.]+\.?)",
        r"(Cancellation[^\n.]+\.?)",
    ]
